...
| 文件 | 作用 |
|---|---|
| `public/ui-patch.css` | 全部 UI 视觉改造：现代化变量契约、分段/按钮/侧栏/搜索/进度条重绘、终端外壳、Maple 字体 `@font-face` + 全局字体覆盖、拖拽区、用量警告条样式、终端图标尺寸等。**靠后加载覆盖，不改原 CSS** |
| `public/themes-patch.js` | 18 套「色所」配色皮肤 + 色卡选择器 + 主题切换猴子补丁（默认电子舞厅）。自注入，不改 app.js 主题逻辑 |
...
| `public/app.js` | 用量面板（官方区恒显+原因+重试+≥85% 警告条+桌面通知+45s 刷新）、快速入口/Agent 增删 UI、`SVG.folder` 形状、`syncMute` 铃铛改 SVG | 中 |
| `server.js` | `/api/roots` 与 `/api/agent-projects` 加 POST 增删 + 持久化（config.json 的 quickRoots/agentHidden/agentPinned）、`claudeOfficialLimits` 返回原因对象、`agentUsage` claudeOut 逻辑 | 中 |
...
## 二、上游更新后重新套用（推荐：git 方式）

全部改动已提交在分支 **`rurutia`** 上（一个提交）。上游出新版后：
